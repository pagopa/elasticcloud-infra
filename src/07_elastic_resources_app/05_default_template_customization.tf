locals {
  ...
  custom_lifecycle_components = { for idx in local.custom_licecycle_idx: idx => jsondecode(templatefile("${path.module}/default_library/index_component/basic-only-lifecycle@custom.json", {lifecycle = var.ilm["default"], name = idx} ))  }
}
