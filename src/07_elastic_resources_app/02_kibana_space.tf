...
resource "elasticstack_kibana_action_connector" "app_connector" {
  ...
  config   = var.app_connectors[each.value.connector].type == "opsgenie" ? jsonencode({ apiUrl = "https://api.opsgenie.com" }) : ( var.app_connectors[each.value.connector].type == "webhook" ? jsonencode({ url = data.azurerm_key_vault_secret.app_connector_secret_key[each.value.connector].value })  : null)
