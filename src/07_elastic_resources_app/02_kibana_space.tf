...
resource "elasticstack_kibana_action_connector" "app_connector" {
  ...
  config   = var.app_connectors[each.value.connector].type == "jira-service-management" ? jsonencode({ apiUrl = "https://api.atlassian.com" }) : (var.app_connectors[each.value.connector].type == "webhook" ? jsonencode({ hasAuth = true, method = "post", url = data.azurerm_key_vault_secret.app_connector_secret_key[each.value.connector].value }) : null)
